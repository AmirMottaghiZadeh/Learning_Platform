...
For K_Game MVP, the primary flashcard entry point is category-based deck creation from active Knowledge Sources. Quiz mistakes remain independent and do not automatically create flashcards.
The flashcard deck selector starts with question type, then optional target category, then Leitner box.
...
- A reviewed card leaves the current due deck immediately; it appears again only when the learner opens the relevant Leitner box or when future scheduling rules reintroduce due reviews.
